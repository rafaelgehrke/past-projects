...
public abstract class GameObj {
    ...
    private void clip() {
        ...
        this.py = Math.min(Math.max(this.py, 300), this.maxY);
    }
    ...
    public boolean intersects(GameObj that) {
        return (this.px + this.width >= that.px + 10
            && this.py + this.height >= that.py + 10
            && that.px + that.width >= this.px + 10
            && that.py + that.height >= this.py + 10);
    }
